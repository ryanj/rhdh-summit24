...
. Two new repositories, link:{gitlab_url}/development/{user}-poi-backend[`{user}-poi-backend`] and link:{gitlab_url}/development/{user}-poi-backend-gitops[`{user}-poi-backend-gitops`], dividing application source from CI/CD automation resources (Figure 3-19).
+
...
The template scaffolding process added an entry to the Software Catalog for the POI backend, {user}-poi-backend. Switch to the Catalog View in the RHDH left navigation to see it (Figure 3-22). 
...
metadata:
  ...
  links:
    ...
    - url: https://console-openshift-console{cluster_id}/k8s/ns/{user}/deployments/{user}-poi-backend
      title: Deployment
      icon: web
    - url: https://devspaces{cluster_id}/#https://gitlab-gitlab{cluster_id}/development/{user}-poi-backend?che-editor=che-incubator/che-code/latest&devfilePath=.devfile-vscode.yaml
...
. who owns it (`{user}`);
. which resource—in this case, database—it depends on (`{user}-poi-backend-db`);
. that it provides an API (`{user}-poi-backend-api`);
. and that this component is part of a system (`idp-system-{user}`).
...
. In GitLab you have two new repositories, namely link:{gitlab_url}/development/{user}-poi-map[`{user}-poi-map`] and link:{gitlab_url}/development/{user}-poi-map-gitops[`{user}-poi-map-gitops`] (Figure 3-42).
+
...
Switch to the Catalog View in Developer Hub to find and explore your two new software catalog components, which are `{user}-poi-map-frontend` (Angular Single Page Application) and `{user}-poi-map-service` (Quarkus proxy), as shown in Figure 3-45.
...
Because you already visited the different tabs for a component’s detail view earlier while exploring the `{user}-poi-backend` component (see link:#_poi_backend_in_the_software_catalog[POI backend in the Software Catalog]) of the POI map application, this should all look very familiar to you at that stage.
